...
namespace Project.Repository
{
    ...
    public class StudentRepository : IStudentRepository
    {
        ...
        public async Task<List<Student>> GetAllStudentsAsync(Sorting sort, Pager page, Filtering filter) {
            ...
            cmd = new SqlCommand("Select * from Student WHERE " + filter.FilterBy + " LIKE  '%" + filter.FilterWord + "%' ORDER BY " + sort.OrderBy + " " + sort.SortOrder+" OFFSET "+((page.PageNumber-1)*page.ItemsPerPage ) +" ROWS FETCH NEXT "+page.ItemsPerPage+ " ROWS ONLY; ", connection);
