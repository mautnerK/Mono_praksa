...
namespace Project.Repository
{
    public class CollegeRepository : ICollegeRepository
    {
              ...
              public async Task<List<College>> GetAllCollegesAsync(Sorting sort, Pager page,Filtering filter)
        {
            ...
            cmd = new SqlCommand("Select * from College"+ "WHERE"+ filter.FilterBy + " LIKE  '%" + filter.FilterWord + "%'" + filter.FilterBy +"  LIKE  '%" + filter.FilterWord + "%'"+" ORDER BY " + sort.OrderBy + " " + sort.SortOrder + " OFFSET " + ((page.PageNumber - 1) * page.ItemsPerPage) + " ROWS FETCH NEXT " + page.ItemsPerPage + " ROWS ONLY; ", connection);
